...
const isHeadless =
	process.env.HEADLESS === "false" ? false : process.env.HEADLESS === "true" || process.env.CI === "true";
...
async function globalSetup(_: FullConfig): Promise<() => Promise<void>> {
	...
	if (missing.length) {
		throw new Error(`Missing required environment variables in e2e/.env.test: ${missing.join(", ")}`);
	}
