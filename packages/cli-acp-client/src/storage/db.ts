/**
 * Sqlite database opener for the CLI's persistent state. Single
 * `state.db` per `cwd`, opened with WAL mode so the host stays
 * read-fast under reentrant agent writes (tool calls + session
 * notification record bursts share a single transaction window).
 *
...
/**
 * Apply schema migrations idempotently. We use a single
 * `schema_version` PRAGMA-style row in a tiny `__migrations` table
 * rather than `user_version` so the surface stays observable from
