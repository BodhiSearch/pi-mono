 ...
 */
import type { VolumeControlReply, VolumeMountReply, VolumeUnmountReply } from './volume-channel';
import type { HostVolumeInit } from './types';

export interface VolumeControl {
  mount(init: HostVolumeInit): Promise<void>;
...
export function createVolumeControl(worker: Worker): VolumeControl {
  ...
  async function send(
    request:
      | { type: 'volumes/mount'; init: HostVolumeInit }
...
export type { HostVolumeInit, VolumeMountReply, VolumeUnmountReply };
