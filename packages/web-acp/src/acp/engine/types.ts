...
import type { BodhiMcpInstanceDescriptor } from '../index';

/**
 * Per-session state owned by the adapter today (Commit 2 of the
 * engine refactor). Will move to `AcpSessionRuntime` in Commit 4 —
 * this interface stays the contract for everything that reads
 * per-session bookkeeping in the meantime.
 */
 ...
 * surface. Commit 4 collapses most of this into `AcpSessionRuntime`.
 */
