 ...
 *
 * We only port the paths web-acp's global-setup needs:
 *
 ...
 * here. Phase A only exercises the public "everything" server, so the
...
export class McpsPage {
  ...
  async createMcpServer(url: string, name: string, description = ''): Promise<void> {
    ...
    await this.page.waitForURL(/\/ui\/mcps\/servers(?!\/new)/);
    // Confirm the list now knows about the server we just created.
  ...
  async createMcpInstance(
    ...
    // Pick the server via its combobox. We reuse the same search-based
    // flow the BodhiApp page object uses, which relies on cmdk item
    // lookups so we don't have to know the server's UUID.
    ...
    // Public auth config — the everything server fixture accepts
    // anonymous Streamable-HTTP calls.
