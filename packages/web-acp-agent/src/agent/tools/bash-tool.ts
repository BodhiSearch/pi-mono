...
import { Bash, type BashExecResult, type IFileSystem, InMemoryFs, MountableFs } from "just-bash/browser";
import type { VolumeRegistry } from "../volume-registry";
...
export function createBashTool(deps: BashToolDeps): AgentTool<typeof bashInputSchema, BashToolDetails> {
	return {
		name: "bash",
		label: "Bash",
...
function truncateStreams(rawStdout: string, rawStderr: string): { stdout: string; stderr: string; truncated: boolean } {
