...
describe("McpConnectionPool", () => {
	...
	it("evicts and reconnects when the auth fingerprint changes", async () => {
		...
		createMcpClientMock.mockResolvedValueOnce(createResult(first)).mockResolvedValueOnce(createResult(second));
	...
	it("emits an error event and rethrows when connect fails", async () => {
		...
		await expect(pool.acquire("s1", serverConfig("everything", "https://example/mcps/ev/mcp"))).rejects.toBe(boom);
	...
	it("releaseAll drops every hold a session has", async () => {
		...
		createMcpClientMock.mockResolvedValueOnce(createResult(a)).mockResolvedValueOnce(createResult(b));
