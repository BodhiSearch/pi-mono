...
export class McpConnectionPool {
  ...
  /**
   * Drop every pool entry whose URL `slugFn` resolves to `slug`,
   * regardless of refcount. Any session sharing the evicted entry loses its
   * connection and must re-`acquire`; this is intentional when toggling a server off.
   */
  async evictBySlug(slug: string, slugFn: (url: string) => string): Promise<void> {
    const target = slug.trim();
    if (!target) return;
    const matches: Array<[string, PoolEntry]> = [];
    for (const [key, entry] of this.#entries) {
      if (slugFn(entry.config.url) === target) matches.push([key, entry]);
    }
    await Promise.all(matches.map(([key, entry]) => this.#evict(key, entry)));
  }
