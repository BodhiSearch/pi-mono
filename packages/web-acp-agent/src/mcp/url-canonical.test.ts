...
describe("canonicalizeMcpUrl", () => {
	it("lowercases the host and drops the default https port", () => {
		expect(canonicalizeMcpUrl("HTTPS://Mcp.Example.COM:443/path")).toBe("https://mcp.example.com/path");
	});

	it("preserves non-default ports", () => {
		expect(canonicalizeMcpUrl("https://mcp.example.com:8443/path")).toBe("https://mcp.example.com:8443/path");
	});
	...
	it("trims surrounding whitespace before parsing", () => {
		expect(canonicalizeMcpUrl("   https://mcp.example.com/mcp   ")).toBe("https://mcp.example.com/mcp");
	});
