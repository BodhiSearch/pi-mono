...
export interface AcpAgentAdapterOptions {
	...
	isDev: boolean;
	/** Build version string of the host runtime; reported by `/version`. */
	buildVersion: string;
 ...
 *
 * Build constants (`isDev`, `buildVersion`, `acpSdkVersion`) used to
 * be Vite `define` globals; the agent package takes them as explicit
 * constructor options so it can be hosted by any runtime (browser
 * worker, Node CLI, etc.) without leaking build-tool assumptions.
 */
export class AcpAgentAdapter implements Agent {
	...
	constructor(conn: AgentSideConnection, services: AcpAdapterServices, options: AcpAgentAdapterOptions) {
