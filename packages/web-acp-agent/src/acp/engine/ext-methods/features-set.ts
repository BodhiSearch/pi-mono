import { isFeatureKey } from "../../../storage/feature-store";
import { BODHI_FEATURES_SET_METHOD, type BodhiFeaturesSetRequest, type BodhiFeaturesSetResponse } from "../../../wire";
import type { ExtMethodHost } from "../types";

export async function featuresSet(params: unknown, host: ExtMethodHost): Promise<BodhiFeaturesSetResponse> {
	const req = params as BodhiFeaturesSetRequest;
	if (!req || typeof req.sessionId !== "string" || typeof req.key !== "string" || typeof req.value !== "boolean") {
		throw new Error(`${BODHI_FEATURES_SET_METHOD}: params must be { sessionId, key, value: boolean }`);
	}
