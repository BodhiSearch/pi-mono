...
export async function getSession(
  params: unknown,
  host: ExtMethodHost
): Promise<BodhiGetSessionResponse> {
  if (!host.store) {
  ...
  // - 'turn' entries carry the cumulative LLM-visible history; we
  //   append the delta from the previous turn's snapshot.
  // - 'builtin' entries are inserted as a tagged user+assistant pair
  //   so reload reproduces them in the right chronological slot.
  //   They never feed `inline.restoreMessages()` because that path
  //   consumes only 'turn' kinds.
