...
import type { AgentEvent, AgentTool, AgentToolResult, AgentToolUpdateCallback } from "@mariozechner/pi-agent-core";
...
import { extractAssistantText, extractMessageId, toolTitle, toToolCallContent } from "../wire-utils";
...
export class PromptTurnDriver {
	...
	async run(params: PromptRequest): Promise<PromptResponse> {
		...
		if (this.#services.streamOverrides) {
			const toolChoice = featureSnapshot.forceToolCall && this.#isDev && tools.length > 0 ? "required" : undefined;
