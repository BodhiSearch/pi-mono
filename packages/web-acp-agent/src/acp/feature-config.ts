...
import { FEATURE_DEFAULTS, type FeatureKey, type FeatureSnapshot } from '../storage/feature-store';
import {
...
// Maps `_bodhi/features/*` config ids to internal feature keys plus
// `SessionConfigOption` metadata. `devOnly` entries are filtered when
// the host reports `isDev === false`.
...
export function buildFeatureConfigOptions(
  ...
  for (const entry of FEATURE_CONFIG_ENTRIES) {
    if (entry.devOnly && !isDev) continue;
    out.push({
      type: 'select',
      id: entry.configId,
      name: entry.name,
