...
| M8  | Extensions: dynamic, toggleable behaviour layer over the agent              | ✅ Phases 1 + 2a | latest | [m8-extensions.md](m8-extensions.md) |
...
- **[m8-extensions.md](m8-extensions.md)** — Phases 1 + 2a landed: vault-scoped `.pi/extensions/<name>/index.js`, Blob-URL dynamic import inside the Worker, the full Phase 1 + Phase 2a hook surface (`before_agent_start`, `tool_result`, `context`, `tool_call`, `turn_start`, `message_end`, `session_loaded` — reload-only), `registerTool` + `registerCommand`, modal `pi.ui.*` channel (`notify`, `setStatus`, `select`, `confirm`, `input`), per-extension toggle + global disable-all trip switch. Full technical reference in [`../specs/worker-agent/extensions.md`](../specs/worker-agent/extensions.md); handoff notes for subsequent phases in [`../extension-impl/`](../extension-impl/) (see the Phase 2a report + Phase 2b prompt).
