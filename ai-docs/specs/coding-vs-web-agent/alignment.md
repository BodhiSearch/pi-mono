...
| System prompt composition | `system-prompt.ts` builds cwd + skills + tool-snippets + extension docs | `system-prompt.ts` — trimmed to cwd + skills + `before_agent_start` extension override (no tool-snippets). See [`../worker-agent/extensions.md`](../worker-agent/extensions.md). |
| Extension lifecycle hooks | `ExtensionRuntime.on(...)` covering `before_agent_start`, `tool_result`, `context`, `tool_call`, `turn_start`, `message_end`, `session_loaded`, `before_compact`, `after_compact` | `ExtensionRunner.emit*` with overloads for every one of those hooks (Phase 1 + Phase 2a + Phase 2b). `session_loaded` carries the same six-value `reason` discriminator (`mount | reload | switch | fork | new | navigate`); `before_compact` is a reducer (returns `{ cutIndex?, preserveEntries? }`) and `after_compact` is an observer. Per-extension error isolation and hook-chain merge semantics match coding-agent exactly. See [`../worker-agent/extensions.md`](../worker-agent/extensions.md). |
| Extension registration surface | `registerTool` / `registerCommand` / `registerProvider` / `registerSkill` on `pi` | `registerTool` / `registerCommand` / `registerProvider` / `registerSkill` on `pi` — extension-contributed providers composite over the base Bodhi provider through `ExtensionProviderController` and extension-contributed skills land in the shared `CommandRegistry` with `source: 'extension-skill'`. |
| Extension UI channel | `pi.ui.notify` / `setStatus` / `setTitle` / `setWidget` / `select` / `confirm` / `input` / `editor` / `setEditorText` backed by pi-tui widgets | `pi.ui.notify` / `setStatus` / `setTitle` / `setWidget` / `select` / `confirm` / `input` / `editor` / `setEditorText` backed by sonner toasts + `ExtensionTitleSlot` + `ExtensionWidgetSlot` + `ExtensionUIRenderer` (Phase 2a + Phase 2b). Extensions written against either runtime share the same call sites for every verb; editor surface differs visually (modal dialog in web vs inline in TUI — see [`divergence.md`](./divergence.md)). |
| Read-only session access for extensions | `ctx.session` on coding-agent extensions exposes a `ReadonlySessionManager` view | `ctx.session` on web-agent extensions exposes a `ReadonlySessionForwarder` that re-reads the live `SessionManager` each call and throws `InvalidSessionError` when the session has been swapped out (prevents stale snapshots across session switches / forks). |
