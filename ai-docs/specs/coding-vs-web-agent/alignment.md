...
| Extension lifecycle hooks | `ExtensionRuntime.on(...)` covering `before_agent_start`, `tool_result`, `context`, `tool_call`, `turn_start`, `message_end`, `session_loaded`, compaction hooks | `ExtensionRunner.emit*` with overloads for the first seven events above (Phase 1 + Phase 2a). Compaction hooks still deferred. Per-extension error isolation and hook-chain merge semantics match coding-agent exactly. See [`../worker-agent/extensions.md`](../worker-agent/extensions.md). |
| Extension UI channel | `pi.ui.notify` / `setStatus` / `setTitle` / `setWidget` / `select` / `confirm` / `input` / `editor` / `setEditorText` backed by pi-tui widgets | `pi.ui.notify` / `setStatus` / `select` / `confirm` / `input` backed by sonner toasts + `ExtensionUIRenderer` (Phase 2a). Widgets, editor, `setTitle`, `setEditorText` deferred to Phase 2b. Extensions written against either runtime can share the same call sites for the modal subset. |
