...
| Extension runtime | `core/extensions/types.ts` (wired via `ExtensionRunner` + TUI) | `core/extensions/{types,loader,runner,wrapper}.ts` + `worker/extension-host.ts` + `worker/extension-ui-controller.ts` + `ExtensionsPanel` / `ExtensionUIRenderer` / `ExtensionStatusChips` + `ExtensionStore` + `useExtensionUI`. Phases 1 + 2a: `before_agent_start` / `tool_result` / `context` / `tool_call` / `turn_start` / `message_end` / `session_loaded` hooks, `registerTool`, `registerCommand`, modal `pi.ui.*` channel (`notify`, `setStatus`, `select`, `confirm`, `input`). Widgets / editor / `setTitle` / provider + skill registration / session access / compaction hooks deferred to Phase 2b. |
...
- **Extension runtime in web-agent.** Phases 1 + 2a landed — `.pi/extensions/<name>/index.js` is discovered, Blob-URL imported inside the Worker, and the hook surface spans `before_agent_start`, `tool_result`, `context`, `tool_call`, `turn_start`, `message_end`, `session_loaded` (reload-only), plus `registerTool` / `registerCommand`. A modal `pi.ui.*` channel (`notify`, `setStatus`, `select`, `confirm`, `input`) rides on an `extension_ui_request` / `extension_ui_response` RPC pair. Slash commands registered by extensions show up with `source: 'extension'`. **Deferred to Phase 2b:** widgets / editor / `setTitle`, `registerProvider`, `registerSkill`, session-manager access, compaction hooks, the initial-mount `session_loaded` lifecycle clarification. **Deferred to Phase 3:** TypeScript sources, iframe-isolated execution, and the marketplace story. See [`../worker-agent/extensions.md`](../worker-agent/extensions.md).
