...
- **web-agent** (M8 — Phases 1 + 2a) runs extensions **inside the same Worker as the agent**. The loader reads `<vaultMount>/.pi/extensions/<name>/index.js` from ZenFS, wraps the string in a `Blob`, and dynamic-`import()`s the resulting URL. Only ESM `export default` entries are supported, bare-specifier imports are unavailable (there is no bundler in the Worker), and the hook surface covers `before_agent_start`, `tool_result`, `context`, `tool_call`, `turn_start`, `message_end`, `session_loaded` (reload-only), plus `registerTool` / `registerCommand`. Phase 2a also landed a modal `pi.ui.*` channel (`notify` → sonner, `setStatus` → chip row, `select` / `confirm` / `input` → `ExtensionUIRenderer` FIFO queue) via a dedicated `extension_ui_request` / `extension_ui_response` RPC pair. Widgets, editor, `setTitle`, `registerProvider`, `registerSkill`, session-manager access, and compaction hooks remain deferred to Phase 2b; iframe-level isolation lands in Phase 3.

**Why:** the Worker has no filesystem resolver for bare specifiers and no child-process story. The Blob-URL trick is the narrowest browser-native equivalent of "load this file as an ES module", and keeping extensions in-Worker (rather than in a dedicated iframe) means hook dispatch stays synchronous with the agent loop. The UI channel has to marshal every call across `postMessage` because React components can't cross the structured-clone boundary — hence the kind-+-props envelope. See [`../worker-agent/extensions.md`](../worker-agent/extensions.md) for the full runtime model, the per-extension error-isolation rules, and the `list_extensions` / `set_extension_states` / `extension_states` / `extension_error` / `extension_ui_request` / `extension_ui_response` RPC surface.
