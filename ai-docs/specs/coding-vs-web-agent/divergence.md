...
- **web-agent** (M8 — Phases 1 + 2a + 2b) runs extensions **inside the same Worker as the agent**. The loader reads `<vaultMount>/.pi/extensions/<name>/index.js` from ZenFS, wraps the string in a `Blob`, and dynamic-`import()`s the resulting URL. Only ESM `export default` entries are supported, bare-specifier imports are unavailable (there is no bundler in the Worker), and the hook surface now covers every coding-agent hook: `before_agent_start`, `tool_result`, `context`, `tool_call`, `turn_start`, `message_end`, `session_loaded` (with the six-value `reason` discriminator `mount | reload | switch | fork | new | navigate`), `before_compact`, `after_compact`, plus the full registration surface `registerTool` / `registerCommand` / `registerProvider` / `registerSkill`. The `pi.ui.*` channel adds `setTitle` (→ `ExtensionTitleSlot` in the chat header), `setWidget` (→ `ExtensionWidgetSlot` rendering the closed `progress | info | choice` enum in the transcript), `editor` (→ modal `EditorDialog` that replaces coding-agent's inline Markdown editor — see "Editor surface" below), and `setEditorText` (fire-and-forget buffer mutation). Extension-contributed providers flow through an `ExtensionProviderController` that composites over the base Bodhi provider and emits `extension_providers_changed` on churn; extension-contributed skills flow through an `ExtensionSkillController` that pushes them to `CommandRegistry` with `source: 'extension-skill'`. `ctx.session` is a thin `ReadonlySessionForwarder` that re-reads the live `SessionManager` every call and throws `InvalidSessionError` when the session has been swapped out, so extensions never hold stale snapshots. Iframe / Worker-per-extension isolation lands in Phase 3.

**Why:** the Worker has no filesystem resolver for bare specifiers and no child-process story. The Blob-URL trick is the narrowest browser-native equivalent of "load this file as an ES module", and keeping extensions in-Worker (rather than in a dedicated iframe) means hook dispatch stays synchronous with the agent loop. The UI channel has to marshal every call across `postMessage` because React components can't cross the structured-clone boundary — hence the kind-+-props envelope. See [`../worker-agent/extensions.md`](../worker-agent/extensions.md) for the full runtime model, the per-extension error-isolation rules, and the `list_extensions` / `set_extension_states` / `extension_states` / `extension_error` / `extension_ui_request` / `extension_ui_response` / `extension_providers_changed` RPC surface.

## Editor surface: modal dialog vs inline Markdown

- **coding-agent** `ctx.ui.editor()` opens an **inline Markdown editor** inside the pi-tui TUI frame: the user stays in the transcript flow, edits in-place, and commits with a key combo. Extensions can stream updates via `set_editor_text` and the TUI repaints the surface incrementally.
- **web-agent** `pi.ui.editor()` opens a **modal dialog** (`ExtensionUIRenderer` → `EditorDialog`) with a textarea + Cancel / Save bar. The dialog takes focus, `Escape` / backdrop-click cancels (resolves `undefined`), Cmd/Ctrl+Enter submits. `setEditorText` mutates the open buffer of the same extension (no-op otherwise). The modal form deliberately reuses the existing FIFO queue that backs `select` / `confirm` / `input` so session-reset cancellation + timeout plumbing are shared across every dialog kind.

**Why:** React's cleanest modal vocabulary is dialog-shaped, and merging editor buffers with the transcript would require a second focus / keyboard-shortcut ownership story. We kept the modal choice explicit in the Phase 2b plan so it can be revisited once UX testing reveals real need for inline editing.

## Compaction hooks: mid-turn heuristic vs reducer-with-observer

- **coding-agent** compaction is driven by `AgentSession.compactIfNeeded` which re-dispatches through the extension runtime internally. Extension hooks plug into the same lifecycle the core loop uses.
- **web-agent** owns compaction in `WorkerAgentHost.runCompaction`, so the 2b integration is a tight two-step: (1) `ExtensionRunner.emitBeforeCompact` runs as a reducer — handlers may return `{ cutIndex?, preserveEntries? }`; later handlers observe earlier overrides, and `prepareCompaction` is re-prepared with the merged outcome before summarisation; (2) `emitAfterCompact` is a pure observer fan-out that receives `{ summary, beforeCount, afterCount }` after the store has committed. Errors are isolated per-handler so a broken extension can't break compaction.
