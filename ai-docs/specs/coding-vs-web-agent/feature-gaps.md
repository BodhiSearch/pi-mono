...
| **Slash-command discovery** (`get_commands` → `RpcSlashCommand[]` with `source: extension|prompt|skill` and `sourceInfo`) | Enumerates registered extension / prompt / skill commands for the UI | Partially ported — `list_commands` RPC returns `SlashCommandInfo[]` with `source: 'builtin' \| 'prompt' \| 'skill'`. The `extension` source is still missing and arrives with the extension runtime. See [`alignment.md § Slash commands`](./alignment.md#slash-commands-prompt-templates-skills). |
...
| **Extension UI request / response channel** (`extension_ui_request` / `extension_ui_response` — select, confirm, input, editor, notify, setStatus, setWidget, setTitle, set_editor_text) | Back-channel for extensions to prompt the TUI | Not implemented (depends on the extension runtime). |
