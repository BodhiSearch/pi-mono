...
| **Extension UI request / response channel** (`extension_ui_request` / `extension_ui_response` — select, confirm, input, editor, notify, setStatus, setWidget, setTitle, set_editor_text) | Back-channel for extensions to prompt the TUI | Phases 2a + 2b landed the full verb set: `notify` → sonner toast, `setStatus` → chip in `ChatInput` footer, `select` / `confirm` / `input` → `ExtensionUIRenderer` modal queue, plus the Phase 2b additions `setTitle` (`ExtensionTitleSlot`), `setWidget` (`ExtensionWidgetSlot` — closed `progress | info | choice` enum), `editor` (modal textarea via `EditorDialog`), and `setEditorText` (fire-and-forget buffer mutation for the open editor). Coding-agent's inline Markdown editor is intentionally replaced with a modal dialog — see [`divergence.md § Editor surface`](./divergence.md#editor-surface-modal-dialog-vs-inline-markdown). Full reference in [`../worker-agent/extensions.md`](../worker-agent/extensions.md). |
...
| **Extension runtime** (full `ExtensionRunner`, `ExtensionRuntime`, command registration, slash-commands, keybindings, widgets, `pi-tui` editors) | Phases 1 + 2a + 2b landed — `packages/web-agent/src/worker-agent/core/extensions/` ships a browser-native runner covering every hook coding-agent exposes (`before_agent_start`, `tool_result`, `context`, `tool_call`, `turn_start`, `message_end`, `session_loaded` with the six-value reason discriminator `mount | reload | switch | fork | new | navigate`, `before_compact`, `after_compact`), the full registration surface (`registerTool`, `registerCommand`, `registerProvider`, `registerSkill`), the complete `pi.ui.*` channel (`notify`, `setStatus`, `setTitle`, `setWidget`, `editor`, `setEditorText`, `select`, `confirm`, `input`), and a read-only `ctx.session` forwarder. Everything loads via Blob-URL dynamic `import()` inline in the agent Worker (see [`../worker-agent/extensions.md`](../worker-agent/extensions.md)). Keybindings, `pi-tui` editors, iframe / Worker-per-extension isolation, bare-specifier imports, and TypeScript sources remain deferred to Phase 3. |
