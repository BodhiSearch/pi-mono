...
| M4  | Worker transport: `AgentSession` + ZenFS run in a Web Worker                | ✅ done  | _this commit_ | +2 vitests (worker transport + structured error round-trip), existing 3 e2e specs unchanged |
...
### M4 — Worker transport (done, _this commit_)
