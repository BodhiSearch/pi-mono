...
**Plan directory convention is now honoured:** `.claude/settings.local.json` sets `plansDirectory: "./ai-docs/plans/"`. This plan file itself lives there. `plans/` is covered by the root `.gitignore` (line 37), so these plan artefacts stay out of git history — steering docs (under `ai-docs/*.md`) are what gets committed.
...
| 3 | Plan storage location | **`./ai-docs/plans/`** per `.claude/settings.local.json`. Gitignored via existing `plans/` entry. |
...
- **Why IndexedDB over OPFS**: OPFS handles do not coordinate across tabs; concurrent writes from multiple open tabs can corrupt state. IndexedDB transactions serialise naturally.
...
- Per-phase re-planning: full plan lives in `ai-docs/plans/*.md` (gitignored); phase-plan.md only carries the index + outcomes.
...
- **Plans live in `ai-docs/plans/`** (gitignored) — one per deliverable, disposable. Steering lives in `ai-docs/*.md` — durable, committed. Decisions live in `ai-docs/05-decisions.md` — append-only.
...
Per-deliverable implementation plans live at `ai-docs/plans/*.md` (gitignored)
and are disposable; the steering docs above are durable.
...
Two stray plan files in the old (wrong) location need to be removed once the plan is approved — they're both duplicates / superseded:

1. `/Users/amir36/.cursor/plans/we-want-to-create-luminous-book.md` — the original Phase 0/1 plan (executed, committed in `06d02b81`). Delete.
2. `/Users/amir36/.cursor/plans/web-agent-steering-docs.md` — early draft of the plan now in `ai-docs/plans/we-want-to-create-luminous-book.md`. Delete.

Neither is tracked by git. `rm` cleanup only.
...
- Adding `plans/` to `.gitignore` — already present at line 37 of root `.gitignore`.
...
- `git status` after all changes shows: 1 modified file (`tsconfig.json` or equivalent — likely zero), new tracked files `CLAUDE.md`, `ai-docs/00-vision.md`, `ai-docs/01-goals.md`, `ai-docs/02-architecture.md`, `ai-docs/03-phase-plan.md`, `ai-docs/04-principles.md`, renamed `ai-docs/05-decisions.md`. No stray `ai-docs/plans/*` should appear in `git status` (confirms the gitignore works).
