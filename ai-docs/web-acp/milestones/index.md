...
| Extension methods    | `_`-prefixed, namespaced                                      | All `_bodhi/*`; see [steering/04-principles.md](../steering/04-principles.md) § 15. `_bodhi/session/get` and its un-prefixed legacy alias `bodhi/getSession` were deleted in the post-2026-05-04 sweep — transcript + toggles ride `LoadSessionResponse._meta.bodhi.{messages, mcpToggles, title}` natively | compliant |
...
| Session listing      | `Agent.listSessions` (stable since 0.20); cursor-paginated | Adopted at M1 + M2 of the ACP-0.21 migration; `bodhi/listSessions` retired. `turnCount` / `lastModelId` / `createdAt` ride `SessionInfo._meta.bodhi`. **Cursor pagination landed post-2026-05-04** — cursor is base64(`page=N&per_page=10&sort_by=updated_at&sort_seq=desc`); `nextCursor` returned until exhausted | compliant |
